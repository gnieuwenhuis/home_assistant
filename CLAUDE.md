...
3. **Per-head lockout** `timer.<room>_head_lockout` — started on every head toggle; the coordinator won't toggle that head again until it expires (office 8 min, studio 6 min). The only override is a hard-safety force-**off** (master-off or backup-heat); lockout never blocks turning a head off, and never permits an on. A safety force-off still arms the lockout, so a quick re-enable or a backup-heat flap can't restart the compressor immediately.
