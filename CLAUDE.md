...
- `head_target(mode, heat_bound, cool_bound, lead)` → the temperature to command a head, a small **lead** (2 °C) *past* the active bound so the inverter stays committed; clamped to `[17, 30]`. The coordinator does the **real cutoff** against the reliable room sensor at the bound, so the lead never causes overshoot past the bound by more than the differential.
...
3. **Per-head lockout** `timer.<room>_head_lockout` — started on every head toggle; the coordinator won't toggle that head again until it expires (office 8 min, studio 6 min). The only override is a hard-safety force-**off** (master-off or backup-heat); lockout never blocks turning a head off, and never permits an on.
4. **Inverter modulation** — commanding a target with the lead and letting the head run (instead of chattering the power switch) lets the compressor ramp down near setpoint.
